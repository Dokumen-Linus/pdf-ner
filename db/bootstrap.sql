...
SELECT format('CREATE ROLE auth_user LOGIN PASSWORD %L', :'AUTH_USER_PASSWORD')
WHERE NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'auth_user')\gexec
SELECT format('ALTER ROLE auth_user LOGIN PASSWORD %L', :'AUTH_USER_PASSWORD')\gexec
...
CREATE SCHEMA IF NOT EXISTS auth AUTHORIZATION owner_role;
...
ALTER SCHEMA auth OWNER TO owner_role;
...
ALTER DEFAULT PRIVILEGES FOR ROLE owner_role IN SCHEMA auth

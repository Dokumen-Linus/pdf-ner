...
CREATE TABLE workers.ner_runs (
  ...
  context_eng_iter_id UUID REFERENCES workers.context_engineering_iterations (id) ON DELETE CASCADE,
  model_eval_iter_id UUID REFERENCES workers.chat_model_eval_iterations (id) ON DELETE CASCADE,
...
CREATE TABLE workers.ner_run_pdfs (
  ...
  pdf_txt_id UUID REFERENCES workers.pdf_txts (id) ON DELETE SET NULL,

...
2. `02_migrate_and_seed.sh` runs Better Auth setup, dbmate migrations, top-level seed SQL, and `standard_entity_types` seed SQL.
3. `03_healthcheck.sh` connects directly as each app role with its own password and verifies key tables and privileges.

## Restarting After Failure

- Better Auth setup is intentionally a one-time block. If `02_migrate_and_seed.sh` fails because auth tables or indexes already exist, comment out the lines between:

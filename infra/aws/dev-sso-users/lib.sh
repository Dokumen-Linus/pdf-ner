...
require_permission_set_has_no_extra_access() {
  local permission_set_arn="$1"
  local allowed_one="$2"
  local allowed_two="$3"
  local aws_managed inline_policy refs ref policy_name policy_path
  ...
  for ref in $refs; do
    ...
    if [ "$policy_path" != "/" ] || { [ "$policy_name" != "$allowed_one" ] && [ "$policy_name" != "$allowed_two" ]; }; then
      echo "ERROR: ${DEV_SSO_PERMISSION_SET_NAME} has unexpected customer managed policy reference ${policy_name} at path ${policy_path}." >&2
      exit 1
    fi
  done
}

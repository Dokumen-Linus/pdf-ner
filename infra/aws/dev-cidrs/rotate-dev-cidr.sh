...
: "${DEV_CIDR:?Set DEV_CIDR to the new dev CIDR block to allow, for example 203.0.113.10/32}"
  ...
  jq -c \
    ...
    '
      ...
      | .Statement[0].Condition.IpAddress["aws:SourceIp"] =
        (
          ([.Statement[0].Condition.IpAddress["aws:SourceIp"]] | flatten | map(select(. != null and . != "" and . != $previous_cidr)))
          + [$new_cidr]
          | unique
        )
    ' <<< "$policy_document"
...
create_or_update_dev_secrets_policy() {
  ...
  if [ -z "$policy_arn" ]; then
    ...
    updated_document="$(policy_document_rotating_cidr "$current_document" "$resources_json" "$previous_cidr" "$new_cidr")"
  ...
  echo "Development secrets read policy replaced ${previous_cidr} with ${new_cidr}: ${policy_arn}"
...
if [ "$PREVIOUS_DEV_CIDR" = "$DEV_CIDR" ]; then
  echo "PREVIOUS_DEV_CIDR and DEV_CIDR are both ${DEV_CIDR}; nothing to rotate."
  exit 0
fi
...
revoke_dev_db_cidr "$DEV_RDS_SECURITY_GROUP_ID" "$RDS_PORT" "$PREVIOUS_DEV_CIDR"
authorize_dev_db_cidr "$DEV_RDS_SECURITY_GROUP_ID" "$RDS_PORT" "$DEV_CIDR"
create_or_update_dev_secrets_policy "$PREVIOUS_DEV_CIDR" "$DEV_CIDR"

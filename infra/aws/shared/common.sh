...
SECRET_DRAFT_DIR="${SECRET_DRAFT_DIR:-${AWS_DIR}/push-secrets/draft-secrets}"
...
configure_common_defaults() {
  ...
  EXAMPLE_SECRETS_DIR="${EXAMPLE_SECRETS_DIR:-${AWS_DIR}/push-secrets/example-secrets}"
  EXAMPLE_SECRETS_DIR="$(absolute_repo_path "$EXAMPLE_SECRETS_DIR")"
  OUTPUT_DIR="${OUTPUT_DIR:-${AWS_DIR}/outputs}"
  ...
  mkdir -p "$EXAMPLE_SECRETS_DIR" "$SECRET_DRAFT_DIR" "$OUTPUT_DIR"
}
...
ensure_secret_drafts() {
  ...
  for name in \
    ...
    if [ ! -f "$target" ]; then
      if [ ! -f "$source" ]; then
        echo "Missing example secret template: $source" >&2
        exit 1
      fi
      cp "$source" "$target"
...
write_env_output() {
  ...
  ensure_output_dir
  {
    echo "# Generated by infra/aws."
    echo "# Resource oIMAGE_OVERRIDE_FILEutput only; scripts discover resources by name and do not read this as input."
    for name in "$@"; do
      if [ "${!name+x}" = "x" ]; then
        printf '%s=%q\n' "$name" "${!name}"
      fi

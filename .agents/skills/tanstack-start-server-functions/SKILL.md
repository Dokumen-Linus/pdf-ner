...
```tsx
import { useServerFn } from '@tanstack/tanstack-react-start'
...
Apply `authMiddleware` (or an equivalent in-handler check) to **every** `createServerFn` that needs auth. See [tanstack-start-core-auth-server-primitives](../tanstack-start-auth-server-primitives/SKILL.md) for the full session/middleware pattern and [tanstack-start-core-middleware](../tanstack-start-middleware/SKILL.md) for composing the factory.
...
- [tanstack-start-core-execution-model](../tanstack-start-execution-model/SKILL.md) — understanding where code runs
- [tanstack-start-core-middleware](../tanstack-start-middleware/SKILL.md) — composing server functions with middleware
- [tanstack-start-core-auth-server-primitives](../tanstack-start-auth-server-primitives/SKILL.md) — sessions, cookies, OAuth, CSRF, rate limiting (the server-side half of auth; `getCurrentUser`/`useSession`-style helpers belong here, not at module scope)
- [tanstack-router/auth-and-guards](../tanstack-router/auth-and-guards/SKILL.md) — the routing side: route guards do NOT protect server functions, so always re-check auth in the handler or via middleware

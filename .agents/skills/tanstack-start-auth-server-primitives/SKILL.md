---
name: tanstack-start-auth-server-primitives
description: >-
  ...
  tanstack-router/auth-and-guards for the routing side.
---

# Auth Server Primitives

This skill covers the **server half** of authentication: session storage, cookie issuance, OAuth flow, password-reset hardening, CSRF, rate limiting. For the **routing half** (`_authenticated` layout, `beforeLoad` redirects, RBAC checks), see [tanstack-router/auth-and-guards](../tanstack-router/auth-and-guards/SKILL.md).
...
import {
  getRequestHeader,
  setResponseHeader,
} from '@tanstack/tanstack-react-start/server'
...
import { createMiddleware } from '@tanstack/tanstack-react-start'
...
```tsx
import { createServerFn } from '@tanstack/tanstack-react-start'
...
```tsx
import { createMiddleware } from '@tanstack/tanstack-react-start'
import { getRequest } from '@tanstack/tanstack-react-start/server'
...
On Cloudflare Workers and other edge runtimes, the module-level read evaluates to `undefined` even on the server because env is injected per-request. See [tanstack-start-core-execution-model](../tanstack-start-execution-model/SKILL.md).
...
- [tanstack-router/auth-and-guards](../tanstack-router/auth-and-guards/SKILL.md) — the routing side: `_authenticated` layout, `beforeLoad`, `redirect`, RBAC checks.
- [tanstack-start-core-server-functions](../tanstack-start-server-functions/SKILL.md) — how to expose RPCs (and how the route guard does NOT cover them).
- [tanstack-start-core-middleware](../tanstack-start-middleware/SKILL.md) — composing `authMiddleware` and others.
- [tanstack-start-core-execution-model](../tanstack-start-execution-model/SKILL.md) — why module-level env/secret reads are wrong.

...
> {
  ...
  private getHandlersForScope(scope: InteractionScope): PointerEventHandlers | null {
    ...
    if (scope.type === "global") {
      ...
      const modeSpecific = mode.scope === "global" ? bucket.global : new Set<PointerEventHandlers>()
      return mergeSets(alwaysSet, modeSpecific)
    }

    const alwaysPageSet =
      this.alwaysPage.get(scope.documentId)?.get(scope.pageIndex) ?? new Set<PointerEventHandlers>()
    const modePageSet =
      mode.scope === "page"

...
import { BufferAndName, DownloadRequestEvent, ExportCapability, ExportScope } from "./types"

export class ExportPlugin extends BasePlugin<BasePluginConfig, ExportCapability> {
  static readonly id = "export" as const

  private readonly downloadRequest$ = createEmitter<DownloadRequestEvent>()
  private readonly config: BasePluginConfig

  constructor(id: string, registry: PluginRegistry, config: BasePluginConfig) {
    super(id, registry)
    this.config = config
  }

...
> {
  ...
  private nextResult(documentId: string): number {
    ...
    const nextIndex =
      docState.activeResultIndex >= docState.results.length - 1 ? 0 : docState.activeResultIndex + 1
  ...
  private previousResult(documentId: string): number {
    ...
    if (!docState || docState.results.length === 0) return -1
    const prevIndex =
      docState.activeResultIndex <= 0 ? docState.results.length - 1 : docState.activeResultIndex - 1

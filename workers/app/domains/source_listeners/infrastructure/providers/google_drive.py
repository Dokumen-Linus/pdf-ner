...
class GoogleDriveListener:
    ...
    async def create(self, connection: dict) -> ListenerProvisioningPayload:
        ...
        page_token = require_config(config, "page_token")
        callback_url = require_config(config, "callback_url")
        channel_id = config.get("channel_id") or str(uuid4())
        body = {
            "id": channel_id,
            "type": "web_hook",
    ...
    async def renew(self, subscription: dict) -> ListenerProvisioningPayload:
        raise NotImplementedError("Google Drive channels are renewed by creating a new channel")
